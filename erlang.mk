...
ifeq ($(PLATFORM),msys2)
EMAKE=C:\\msys64\\usr\\bin\\$(MAKE).exe
else
EMAKE=$(MAKE)
endif
						...
						{_, Deps} ->
							case lists:keyfind(P, 1, Deps) of
								...
								_ ->
									...
									io:format("~s", [os:cmd("$(EMAKE) -C $(call core_native_path,$(DEPS_DIR)/$1) " ++ Path)]),
									io:format("~s", [os:cmd("$(EMAKE) -C " ++ Path ++ " IS_DEP=1")]),
...
$(DEPS_DIR)/$(call dep_name,$1):
	...
	$(verbose) if [ ! -d $(DEPS_DIR)/$(DEP_NAME) ]; then \
		mkdir -p $(DEPS_DIR); \
		$(call dep_fetch_$(strip $(call dep_fetch,$(1))),$(1)) \
	fi
...
ifeq ($(filter $(1),$(NO_AUTOPATCH)),)
	$(verbose) $$(call dep_autopatch,$(DEP_NAME))
endif
endef

$(foreach dep,$(BUILD_DEPS) $(DEPS),$(eval $(call dep_target,$(dep))))

.PHONY: autopatch-deps
autopatch-deps: $(foreach dep,$(BUILD_DEPS) $(DEPS),$(abspath $(DEPS_DIR)/$(dep)))
