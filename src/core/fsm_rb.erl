...
-export([last_no/0,tail/0,tail/1,tail_after/1]).
-export([h/0, help/0]).
...
-record(state, {dir, data, order, device, max, type, abort, log}).
...
start(Options) ->
    supervisor:start_child(sasl_sup, 
			   {rb_server, {fsm_rb, start_link, [Options]},
			    temporary, brutal_kill, worker, [rb]}).

start_link(Options) ->
    gen_server:start_link({local, rb_server}, fsm_rb, Options, []).
...
list_tail(Type,N) -> call({list, Type, N}).
...
show_tail(Type,N) when is_atom(Type);is_list(Type) ->
    call({show_type, Type, N}).
...
help() ->
    ...
    io:format("===========================~n"),
    io:format("fsm_rb:start()         - start the rb_server with default options~n"),
    io:format("fsm_rb:start(Options)  - where Options is a list of:~n"),
    print_options(),
    io:format("fsm_rb:h()             - print this help~n"),
    io:format("fsm_rb:help()          - print this help~n"),
    io:format("fsm_rb:list()          - list all reports~n"),
    io:format("fsm_rb:list(Type)      - list all reports of type Type~n"),
    io:format("fsm_rb:log_list()      - log list of all reports~n"),
    io:format("fsm_rb:log_list(Type)  - log list of all reports of type Type~n"),
    io:format("      currently supported types are:~n"),
    print_types(),
    io:format("fsm_rb:grep(RegExp)      - print reports containing RegExp.~n"),
    io:format("                     RegExp must be a valid argument for ~n"),
    io:format("                     the function re:run/2 or re:run/3.~n"),
    io:format("fsm_rb:filter(Filters) - print reports matching Filters.~n"),
    io:format("                     reports must be proplists.~n"),
    io:format("      Filters is a list of tuples of the following form:~n"),
    print_filters(),
    io:format("fsm_rb:filter(Filters, Dates)  -~n"),
    io:format("      same as fsm_rb:filter/1 but accepts date ranges to filter reports.~n"),
    io:format("      Dates must be of the following form:~n"),
    print_dates(),
    io:format("fsm_rb:rescan()        - rescans the report directory with same~n"),
    io:format("                     options.~n"),
    io:format("fsm_rb:rescan(Options) - rescans the report directory with new~n"),
    io:format("                     options. Options is same as in start/1.~n"),
    io:format("fsm_rb:show(Number)    - print report no Number~n"),
    io:format("fsm_rb:show(Type)      - print all reports of type Type~n"),
    io:format("fsm_rb:show()          - print all reports~n"),
    io:format("fsm_rb:start_log(File) - redirect all reports to file or io_device~n"),
    io:format("fsm_rb:stop_log()      - close the log file and redirect to~n"),
    io:format("                     standard_io~n"),
    io:format("fsm_rb:stop            - stop the rb_server~n").
...
init(Options) ->
    ...
    {ok, #state{dir = Dir ++ "/", data = Data, order = direct, device = Device,
...
handle_call({rescan, Options}, _From, State) ->
    ...
    NewState = State#state{data = Data, order = direct, max = Max, type = Type,
			   device = Device, abort = Abort, log = Log1},
    {reply, ok, NewState};
handle_call(_, _From, #state{data = undefined}) ->
    {reply, {error, no_data}, #state{}};
handle_call({list, Type}, _From, State) ->
    #state{data = Data} = direct_order(State),
    print_list(standard_io, Data, Type),
    {reply, ok, State};
handle_call({list, Type, N}, _From, State) ->
    #state{data = Data} = reverse_order(State),
    print_list(standard_io, lists:reverse(lists:sublist(Data,N)), Type),
    {reply, ok, State#state{data = Data, order = reverse}};
...
handle_call({show_number, Number}, _From, State) ->
    #state{dir = Dir, data = Data, device = Device, abort = Abort, log = Log} = direct_order(State),
...
handle_call({show_type, Type}, _From, State) ->
    ...
    {reply, ok, State#state{device = NewDevice}};
handle_call({show_type, Types, N}, _From, State) ->
    #state{dir = Dir, data = Data, device = Device, abort = Abort, log = Log} = reverse_order(State),
    I = count_typed_reports(Data, Types, N, 0),
    NewDevice = print_typed_reports(Dir, lists:reverse(lists:sublist(Data,I)), Types, Device, Abort, Log),
    {reply, ok, State#state{device = NewDevice, data = Data, order = reverse}};
...
handle_call({grep, RegExp, N}, _From, State) ->
    #state{dir = Dir, data = Data, device = Device, abort = Abort, log = Log} = reverse_order(State),
    I = count_grep_reports(Dir, Data, RegExp, N, 0),    
    try print_grep_reports(Dir, lists:reverse(lists:sublist(Data,I)), RegExp, Device, Abort, Log) of
	NewDevice ->
	    {reply, ok, State#state{device = NewDevice}}
    catch
	error:Error ->
...
handle_call({grep_after, RegExp, N}, _From, State) ->
    #state{dir = Dir, data = Data, device = Device, abort = Abort, log = Log} = reverse_order(State),
    {Last_no,_,_,_,_,_} = hd(Data),
    try print_grep_reports(Dir, lists:reverse(lists:sublist(Data,N-Last_no)), RegExp, Device, Abort, Log) of
	NewDevice ->
	    {reply, Last_no, State#state{device = NewDevice}}
    catch
	error:_Error ->
	    {reply, Last_no, State}
    end;
...
%% FIXME: handle data = undefined case!
handle_call({add_report, [Bin, Offset, File]}, _From, #state{data = Data, order = Order} = State) ->
    RData = case Order of
		direct -> lists:reverse(Data);
		_ -> Data
	    end,
    Last = hd(RData),
...
handle_call(last_no, _From, State) ->
    ...
    {Last_no,_,_,_,_,_} = hd(Data),
    {reply, Last_no, State#state{device = Device, data = Data, order = reverse}};
handle_call({tail, N}, _From, State) ->
    #state{dir = Dir, data = Data, device = Device, abort = Abort, log = Log} = reverse_order(State),
    {Last_no,_,_,_,_,_} = hd(Data),
    NewDevice = lists:foldl(fun(Number,NDevice) ->
				    print_report_by_num(Dir, Data, Number, NDevice, Abort, Log)
			    end, Device, lists:reverse(lists:seq(Last_no,Last_no+N))),
    {reply, Last_no, State#state{device = NewDevice, data = Data, order = reverse}};
handle_call({tail_after, N}, _From, State) ->
    %% FIXME: дублирование кода 
    #state{dir = Dir, data = Data, device = Device, abort = Abort, log = Log} = reverse_order(State),
    {Last_no,_,_,_,_,_} = hd(Data),
    NewDevice = lists:foldl(fun(Number,NDevice) ->
				    print_report_by_num(Dir, Data, Number, NDevice, Abort, Log)
			    end, Device, lists:reverse(lists:seq(Last_no,N-1))),
    {reply, Last_no, State#state{device = NewDevice, data = Data, order = reverse}}.
    
direct_order(#state{order = direct} = State) -> State;
direct_order(#state{data = Data, order = reverse} = State) -> State#state{data = lists:reverse(Data), order = direct}.

reverse_order(#state{order = reverse} = State) -> State;
reverse_order(#state{data = Data, order = direct} = State) -> State#state{data = lists:reverse(Data), order = reverse}.
...
open_log_file(Fd) when is_atom(Fd),Fd=/=standard_error -> 
    case whereis(Fd) of
	undefined -> io:format("fsm_rb: Registered name not found '~s'.~n",
		      [Fd]),
	             io:format("fsm_rb: Using standard_io~n"),
...
open_log_file(FileName) when is_list(FileName) ->
	...
	{ok, Fd} -> Fd;
	Error -> 
	    io:format("fsm_rb: Cannot open file '~s' (~w).~n",
		      [FileName, Error]),
	    io:format("fsm_rb: Using standard_io~n"),
	    standard_io
    end;
...
read_reports(No, Fd, Fname, Max, Type) ->
    io:format("fsm_rb: reading report..."),
...
print_list(Fd, Data, Type) ->
    ...
    Format = lists:concat(["~5s~19s ~", Width, "s~20s~n"]),
...
print_short_descr(Fd, No, Type, ShortDescr, Date, Width, DateWidth) ->
    Format = lists:concat(["~5w~20w ~", Width, "s~", DateWidth,"s~n"]),
...
count_typed_reports([], _, _, I) -> I;
count_typed_reports(_, _, N, I) when N =< 0 -> I;
count_typed_reports([H|T], Types, N, I) ->
    case element(2, H) of
	Type when is_atom(Types), Types == Type -> count_typed_reports(T, Types, N-1, I+1);
	Type when is_list(Types) ->
	    case lists:member(Type,Types) of
		true -> count_typed_reports(T, Types, N-1, I+1);
		_ -> count_typed_reports(T, Types, N, I+1)
	    end;
	_ -> count_typed_reports(T, Types, N, I+1)
    end.

print_typed_reports(_Dir, [], _Types, Device, _Abort, _Log) ->
    Device;
print_typed_reports(Dir, Data, Types, Device, Abort, Log) ->
    {Next,Device1} = 
	case element(2, hd(Data)) of
	    Type when is_atom(Types), Types == Type -> 
		print_report(Dir, Data, element(1, hd(Data)), Device, Abort, Log);
	    Type when is_list(Types) ->
		case lists:member(Type,Types) of
		    true -> print_report(Dir, Data, element(1, hd(Data)), Device, Abort, Log);
		    _ -> {proceed,Device}
		end;
	    _ -> 
		{proceed,Device}
    ...
    if Next == abort ->
	    Device1;
       true ->
	    print_typed_reports(Dir, tl(Data), Types, Device1, Abort, Log)
    end.
...
print_report(Dir, Data, Number, Device, Abort, Log) ->
	...
	{Fname, FilePosition} ->
		...
		_ -> 
		    io:format("fsm_rb: can't open file ~p~n", [Fname]),
		    {proceed,Device}
	    end;
	no_report ->
	    {proceed,Device}
...
count_grep_reports(_, [], _, _, I) -> I;
count_grep_reports(_, _, _, N, I) when N =< 0 -> I;
count_grep_reports(Dir, [H|T], RegExp, N, I) ->
    Increment = count_grep_report(Dir, H, RegExp),
    count_grep_reports(Dir, T, RegExp, N - Increment, I + 1).

count_grep_report(Dir, Data, RegExp) ->
    {_, _, _, _, Fname, FilePosition} = Data,
    FileName = lists:concat([Dir, Fname]),
    case file:open(FileName, [read]) of
...
check_rep(Fd, FilePosition, Device, RegExp, Number, Abort, Log) ->
	...
	{Date, Msg} ->
		...
		match ->
		    case Msg of
			{_Time, {FSM, _, _}} when FSM == fsm_progress; FSM == fsm_core; FSM == fsm_event; FSM == fsm_cast; FSM == fsm_transition -> nothing;
			_ -> io:format("Found match in report number ~w~n", [Number])
		    end,
		    case catch fsm_rb_format_supp:print(Date, Msg, Device) of
			{'EXIT', _} ->
	...
	_ ->
	    io:format("fsm_rb: Cannot read from file: Fd ~p, FilePos: ~p~n", [Fd, FilePosition]),
	    {proceed,Device}
    end.
...
filter_rep({Filters, FDates}, Fd, FilePosition, Device, Abort, Log) ->
	...
	_ ->
	    io:format("fsm_rb: Cannot read from file~n"),
	    {proceed,Device}
    end;
filter_rep(Filters, Fd, FilePosition, Device, Abort, Log) ->
    RepMsg = read_rep_msg(Fd, FilePosition),
...
read_rep(Fd, FilePosition, Device, Abort, Log) ->
    case read_rep_msg(Fd, FilePosition) of
	{Date, Msg} ->
	    %% fsm_rb_format_supp:print(Date, Msg, Device),
	    case catch fsm_rb_format_supp:print(Date, Msg, Device) of
	    	{'EXIT', _} ->
	    	    handle_bad_form(Date, Msg, Device, Abort, Log);
	    	_ ->
...
handle_bad_form(Date, Msg, Device, Abort, Log) ->
    io:format("fsm_rb: ERROR! A report on bad form was encountered. " ++
	...
	{true,standard_io,standard_io} ->
	    io:format("fsm_rb: Logging aborted.~n"),
	    {abort,Device};
	{false,standard_io,standard_io} ->
	    io:format("fsm_rb: Logging resumed...~n~n"),
	    {proceed,Device};
	{_,_,standard_io} ->
	    io:format("fsm_rb: Can not reopen ~p. Logging aborted.~n", [Log]),
	    {abort,Device};
	{true,_,NewDevice} ->
	    io:format(NewDevice,
		      "~n~n************************* RB ERROR ************************~n" ++
	...
	{false,_,NewDevice} ->
	    io:format(NewDevice, 
		      "~n   ********* FSM_RB: UNPRINTABLE REPORT ********~n~n", []),
	    io:format("fsm_rb: Logging resumed...~n~n"),	    
	    {proceed,NewDevice}
    end.
